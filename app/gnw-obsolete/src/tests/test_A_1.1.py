...
#        print v.name, "=", v.varValue
    ...
    for t in xrange( nSteps + 1):
        ...
        mkt_in_flow     = market.name   + "_iflow_%d" % t
        mkt_out_flow    = market.name   + "_oflow_%d" % t
        if t < nSteps:
            print "%15d; %15.12f; %15.12f; %15.12f; %15.2f; %15.2f;" %(t, lp.variablesDict()[strg_lev_pct].varValue, lp.variablesDict()[strg_q_inj].varValue, lp.variablesDict()[strg_q_rel].varValue, lp.variablesDict()[mkt_in_flow].varValue, lp.variablesDict()[mkt_out_flow].varValue)
        else:
